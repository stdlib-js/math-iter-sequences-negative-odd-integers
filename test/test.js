...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterNegativeOddIntegersSeq, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not an object', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			iterNegativeOddIntegersSeq( value );
		};
	}
});
...
tape( 'the function returns an iterator protocol-compliant object which generates a sequence of negative odd integers', function test( t ) {
	var expected;
	var actual;
	var it;
	var i;

	it = iterNegativeOddIntegersSeq();
	t.strictEqual( it.next.length, 0, 'has zero arity' );

	for ( i = 0; i < 1000; i++ ) {
		actual = it.next();
		expected = -((i*2) + 1);
		t.strictEqual( actual.value, expected, 'returns expected value' );
		t.strictEqual( isPositiveInteger( actual.value ), true, 'returns a negative integer' );
		t.strictEqual( isEven( actual.value ), true, 'returns an odd integer' );
		t.strictEqual( actual.done, false, 'returns expected value' );
	}
	t.end();
});
...
tape( 'the function supports limiting the number of iterations', function test( t ) {
	...
	it = iterNegativeOddIntegersSeq( opts );
	t.strictEqual( it.next.length, 0, 'has zero arity' );
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var iterNegativeOddIntegersSeq; // eslint-disable-line id-length
	var it1;
	var it2;
	var i;

	iterNegativeOddIntegersSeq = proxyquire( './../lib/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	it1 = iterNegativeOddIntegersSeq();
